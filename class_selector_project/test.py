...
# Get the links to all the deparments
def get_links(): 
    pageHTML = html.fromstring(urlopen(url).read())
    soup = pageHTML.xpath('.//td[@class="block_content"]')[0].text_content()    
    
    html = urlopen('http://www.macalester.edu/registrar/schedules/2015spring/class-schedule/').read()
    links = pageHTML.xpath('.//a[@class="more"]/@href')
    #links.remove('')
    links.remove('http://www.macalester.edu/academic/catalog/program/ped_m.html')
    #soup = BeautifulSoup(html)
    #soup = soupparser.parse(html)
...
def all_major_dict():
    ...
    for i in range(len(link_list)):
        ...
        all_h3 = soup.find_all(text = re.compile(majors[i])) #get the names of the classes in that major
        inside_dict = {}
